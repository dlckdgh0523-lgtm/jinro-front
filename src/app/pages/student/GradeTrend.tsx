...
export function GradeTrend() {
  ...
  // API에서 모든 모드 데이터를 한 번에 로드
  const fetchAll = async () => {
    setIsLoading(true);
    try {
      const [finalData, examData, practiceData] = await Promise.all([
      ...
      // API가 lines에 color를 포함하지 않을 경우 보정
      const normalize = (r: ChartResult): ChartResult => ({
        ...r,
        lines: r.lines.map((l, i) => ({ ...l, color: l.color || getSubjectColor(l.label, i) })),
      });

      const newMap = {
        final:    normalize(finalData),
        exam:     normalize(examData),
        practice: normalize(practiceData),
      };
      setDataMap(newMap);
      setSelectedSubjects(newMap[chartModeRef.current].lines.map(l => l.dataKey));
    } catch {
      // 오류 시 빈 데이터 유지
    } finally {
      setIsLoading(false);
  ...
  return (
    <div>
      ...
      {/* ── 실제 데이터 없음 안내 ─────────────────────────────────────────────── */}
      {!isLoading && !currentData.hasRealData && (
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-700 rounded-xl px-4 py-3 mb-5 flex items-center justify-between gap-3">
          <p className="text-xs text-amber-700 dark:text-amber-400">
            ⚠️ {config.noDataMsg}&nbsp;<strong>{config.noDataHint}</strong>
          </p>
          <button
          ...
          <div className="bg-card rounded-xl border border-border overflow-hidden">
            ...
            {currentData.data.length === 0 ? (
              <div className="px-5 py-8 text-center">
                <p className="text-sm text-muted-foreground">{config.noDataMsg}</p>
              </div>
            ) : (
